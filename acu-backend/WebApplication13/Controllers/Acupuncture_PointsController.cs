...
namespace WebApplication13.Controllers
{
    ...
    public class Acupuncture_PointsController : ControllerBase
    {
        ...
        public IActionResult NewAcupuncture_Points([FromForm]  Acupuncture_PointsViewModel value)
        {
        ...
        #region 修改穴位資訊
        [HttpPut]
        [Route("PutAcupuncture_Points/{acupuncture_points_id}")]
        public IActionResult PutAcupuncture_Points(Guid acupuncture_points_id, [FromForm]  UpdateAcupuncture_PointsViewModel value)
        {
            var result = _acupuncture_pointsService.PutAcupuncture_Points(acupuncture_points_id, value);
            if (result == null)
            {
                return NotFound("找不到資源");
            }
        ...
        #region 總覽個別穴位資訊
        [HttpGet]
        [Route("GetAcupuncture_Points_Infromation/{acupuncture_points_id}")]
        public IActionResult GetAcupuncture_Points_Infromation([FromRoute]Guid acupuncture_points_id)
        {
            var result = _acupuncture_pointsService.GetAcupuncture_Points_Infromation(acupuncture_points_id);
            if (result == null || result.Count <= 0)
            {
                return NotFound("找不到資源");
            }
        ...
        #region 詳細穴位資訊API(欄位:圖片、資料庫所有、是否完成復建)
        [HttpGet]
        [Route("GetAcupuncture_Points")]
        public IActionResult GetAcupuncture_Points(Guid user_id, DateTime R_record_date)
        {
            var result = _acupuncture_pointsService.GetAcupuncture_Points(user_id, R_record_date);
            if (result == null || result.Count <= 0)
            {
                return NotFound("找不到資源");
            }
