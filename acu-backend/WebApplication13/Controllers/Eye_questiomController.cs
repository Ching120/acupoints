...
namespace WebApplication13.Controllers
{
    ...
    public class Eye_questionController : ControllerBase
    {
        ...
        #region 修改檢測題目
        [HttpPut]
        [Route("PutEye_Question/{eye_question_id}")]
        public IActionResult PutEye_Question([FromRoute] Guid eye_question_id, UpdateEye_questionViewModel value)
        {
            var result = _eye_questionService.PutEye_Question(eye_question_id, value);
            if (result == null)
            {
                return NotFound("找不到資源");
            }
