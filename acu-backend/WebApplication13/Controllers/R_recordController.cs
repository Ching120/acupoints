...
namespace WebApplication13.Controllers
{
    ...
    public class R_recordController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("NewR_record/{acupuncture_points_id}/{user_id}")]
        public IActionResult NewR_record([FromRoute]Guid acupuncture_points_id,[FromRoute] Guid user_id)
        {
        ...
        #region 修改穴位復健紀錄
        [HttpPut]
        [Route("PutR_record/{user_id}/{R_record_date}")]
        public IActionResult PutR_record([FromRoute]Guid user_id,[FromRoute] DateTime R_record_date)
        {
            var result = _r_recordDBService.PutR_record(user_id,R_record_date);
            if (result == null)
            {
                return NotFound("找不到資源");
            }
        ...
        [HttpGet]
        [Route("GetR_record/{user_id}/{R_record_date}")]
        public IActionResult GetR_record([FromRoute] Guid user_id,[FromRoute] DateTime R_record_date)
        {
