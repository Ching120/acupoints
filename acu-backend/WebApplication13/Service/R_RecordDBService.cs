    ...
    public class R_RecordDBService
    {
        ...
        public string NewR_record(Guid acupuncture_points_id, Guid user_id)
        {
            ...
            string sql = $@"INSERT INTO R_record
                        ...
                        VALUES (@R_record_id,@user_id,@R_record_date,@acupuncture_points_id,@R_record_finish,@isdel,@create_id,@create_time,@update_id,@update_time)";
        ...
        #region 修改穴位復健紀錄
        public string PutR_record(Guid user_id,DateTime R_record_date)
        {

            string sql = $@"UPDATE R_record  SET R_record_finish=@R_record_finish,update_id=@update_id,update_time=@update_time WHERE user_id = @user_id and R_record_date= @R_record_date";
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    command.Parameters.AddWithValue("@R_record_finish", 0);
                    command.Parameters.AddWithValue("@update_time", DateTime.Now);
                    command.Parameters.AddWithValue("@update_id", "admin");
        ...
        {
            string sql = @"
            SELECT R_record.*, acupuncture_points.*
            FROM R_record
